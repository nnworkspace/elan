...
- **Mandate:** We implement the *Participant Onboarding Rules* defined in:
    - `@rule=SET-RULEBOOK:0.9.0` (specifically [`onboarding-rules.md`](../../30-rulebook/onboarding-rules.md))
...
- **Test Suite:** The logic here is verified by the Conformance Test Suite:
    - `@test=TEST-SET-ONB:0.1.1` (located in `70-tests/100-conformance-scenarios/onboarding/`)
...
- **Implementation:** The Reference Implementation derives its logic from here:
    - `80-code/adapter-service` (Must declare compliance to `SPEC-SET-ONB:0.1.1`)
