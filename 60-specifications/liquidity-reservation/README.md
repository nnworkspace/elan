...
- **Mandate:** We implement the *Liquidity & Funding Rules* defined in:
    - `@rule=SET-RULEBOOK:0.9.0` (specifically [`liquidity-and-waterfall.md`](../../30-rulebook/liquidity-and-waterfall.md))
...
- **Test Suite:** The logic here is verified by the Conformance Test Suite:
    - `@test=TEST-SET-LIQ:0.1.0` (located in `70-tests/100-conformance-scenarios/liquidity/`)
...
- **Implementation:** The Reference Implementation derives its logic from here:
    - `80-code/waterfall-engine` (Must declare compliance to `SPEC-SET-LIQ:0.1.0`)
