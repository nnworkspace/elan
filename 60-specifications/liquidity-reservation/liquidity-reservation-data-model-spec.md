...
This document defines the **schema and data integrity rules** for the Liquidity Engine (`COMP-PSP-02`).
...
1.  **Persistence Requirement (`COMP-PSP-02`):** The Liquidity Engine must maintain a persistent record (`LiquidityReservation`) of every commercial lock. This ensures that if the system crashes after locking funds but before funding, the recovery process can identify the "Orphan Lock" and either complete or void it (Satisfying `REQ-LIQ-02`).
...
| **DAT-PRI-01** | **Opaque References** | The `reservation_proof` sent to DESP MUST NOT contain PII or raw Core Banking IDs. It must be a random UUID or Hash generated by the Liquidity Engine. | `PRIV-01` |
