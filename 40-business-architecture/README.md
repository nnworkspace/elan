...
- **Upstream (`30-rulebook`):** every business-architecture choice exists to satisfy a rule. Architectural artefacts reference the specific **Rulebook Set Version** (e.g. `@rule=SET-RULEBOOK:0.9.0`).
- **Downstream (`50-system-architecture`):** the system architecture realises this business architecture. The components in Layer 50 must trace back to the interactions, responsibilities, and data boundaries defined here.
