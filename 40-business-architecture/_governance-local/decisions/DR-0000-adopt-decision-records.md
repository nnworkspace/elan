...
# DR-0000: Adopt decision records for the business architecture layer

| Field         | Value                 |
| :------------ | :-------------------- |
| Status        | Accepted              |
| Date          | 2026-05-26            |
| Deciders      | Business Architecture |
| Supersedes    | none                  |
| Superseded by | none                  |

## Context and problem statement

The business architecture layer accumulates decisions that are costly to reverse and specific to this layer: where trust boundaries are drawn, which actor sees which data, how the privacy firewall is partitioned. The reasoning behind such decisions is easily lost once the discussion that produced it has faded. The project has adopted decision records as the standard mechanism for preserving that reasoning (see [`PDR-0000`](../../../00-project-governance/decisions/PDR-0000-adopt-decision-records.md)), and this layer needs a local place to keep the records that belong to it.
