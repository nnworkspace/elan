...
The project adopts **decision records** as defined in [`decision-records.md`](../decision-records.md). Decisions that cut across repositories are recorded as `PDR-` files in `00-project-governance/decisions/`; decisions confined to a single repository are recorded as `DR-` files in that repository's own `decisions/` folder. Records are immutable once accepted; a decision is changed by writing a new record that supersedes the old one.
