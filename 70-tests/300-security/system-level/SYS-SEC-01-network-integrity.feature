...
Feature: System-Wide Network Integrity & Zone Isolation
  ...
  Governing rule:  Zone isolation and defence in depth (ARCH-SEC-01 / ARCH-SEC-02)
  Behaviour:       Network segmentation and transport security enforce zone
                   boundaries so a compromise in one zone cannot trivially
                   escalate to another. The Access Gateway accepts no
                   connection without a valid QWAC; Zone B is unroutable from
                   the public internet; and every state-changing operation
  ...
  Scenario: The gateway rejects connections without a client certificate
